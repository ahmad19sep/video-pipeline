...
from cutmachine.learning import learning_summary
...
def _html_page(
    ...
    learning = learning_summary(context.repository_root)
...
<style>
:root{{--bg:#050914;--panel:rgba(15,27,48,.86);--line:rgba(112,150,196,.28);--text:#f3f8ff;--muted:#9db0c8;--accent:#67e8f9;--accent2:#8b5cf6;--ok:#86efac;--warn:#fde68a;--bad:#fca5a5;--shadow:0 24px 70px rgba(0,0,0,.34)}}
*{{box-sizing:border-box}}html{{scroll-behavior:smooth}}body{{margin:0;background:radial-gradient(circle at 80% 0,rgba(103,232,249,.12),transparent 28rem),radial-gradient(circle at 12% 22%,rgba(139,92,246,.12),transparent 24rem),var(--bg);color:var(--text);font:16px/1.55 Inter,Segoe UI,Arial,sans-serif}}body:before{{content:"";position:fixed;inset:0;pointer-events:none;background-image:linear-gradient(rgba(255,255,255,.018) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,.018) 1px,transparent 1px);background-size:48px 48px;mask-image:linear-gradient(to bottom,#000,transparent 75%)}}main{{position:relative;max-width:1220px;margin:auto;padding:32px 24px 96px}}header.hero{{position:relative;overflow:hidden;border:1px solid var(--line);border-radius:28px;padding:36px;background:linear-gradient(135deg,rgba(16,29,49,.96),rgba(9,19,36,.82));box-shadow:var(--shadow)}}header.hero:after{{content:"";position:absolute;width:260px;height:260px;border-radius:50%;right:-80px;top:-120px;background:var(--accent);filter:blur(90px);opacity:.16}}h1,h2,h3{{line-height:1.12}}h1{{font-size:clamp(2.1rem,6vw,4.8rem);letter-spacing:-.055em;margin:.2em 0}}h2{{margin-top:54px;font-size:clamp(1.45rem,3vw,2.25rem);letter-spacing:-.03em}}h3{{letter-spacing:-.015em}}.eyebrow{{color:var(--accent);font-size:.78rem;font-weight:800;letter-spacing:.18em;text-transform:uppercase}}.badge{{display:inline-flex;align-items:center;padding:6px 12px;border-radius:999px;background:{"var(--ok)" if status == "passed" else "var(--bad)"};color:#07111f;font-size:.78rem;font-weight:900;letter-spacing:.08em}}nav{{position:sticky;top:12px;z-index:5;display:flex;gap:8px;margin:18px 0 30px;padding:10px;overflow:auto;border:1px solid var(--line);border-radius:16px;background:rgba(5,9,20,.82);backdrop-filter:blur(18px);box-shadow:0 14px 40px rgba(0,0,0,.25)}}nav a{{color:var(--muted);text-decoration:none;white-space:nowrap;padding:8px 12px;border-radius:10px;font-size:.88rem;font-weight:700}}nav a:hover{{background:rgba(103,232,249,.1);color:var(--accent)}}section{{scroll-margin-top:92px}}video{{display:block;width:min(100%,820px);max-height:72vh;background:#000;border:1px solid var(--line);border-radius:22px;box-shadow:var(--shadow)}}.grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:16px}}.card{{background:linear-gradient(145deg,var(--panel),rgba(10,21,39,.9));border:1px solid var(--line);border-radius:18px;padding:18px;overflow-wrap:anywhere;box-shadow:0 16px 44px rgba(0,0,0,.18)}}.card strong{{display:inline-block;margin-bottom:5px;color:var(--accent);font-size:.78rem;letter-spacing:.08em;text-transform:uppercase}}.card img,.compare img{{width:100%;border-radius:12px;background:#000}}.compare{{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:18px}}figure{{margin:0;padding:12px;border:1px solid var(--line);border-radius:18px;background:var(--panel)}}figcaption{{padding:8px 4px 2px;color:var(--muted);font-weight:700}}table{{width:100%;border-collapse:separate;border-spacing:0;overflow:hidden;background:var(--panel);border:1px solid var(--line);border-radius:16px}}th,td{{padding:12px;border-bottom:1px solid var(--line);text-align:left;vertical-align:top}}tr:last-child td{{border-bottom:0}}th{{color:var(--accent);font-size:.78rem;letter-spacing:.07em;text-transform:uppercase}}.warning td:first-child{{color:var(--warn)}}.error td:first-child{{color:var(--bad)}}code{{color:var(--accent)}}.muted{{color:var(--muted)}}#action{{margin-top:58px;padding:28px;border:1px solid rgba(103,232,249,.35);border-radius:22px;background:linear-gradient(130deg,rgba(103,232,249,.1),rgba(139,92,246,.08))}}@media(max-width:640px){{main{{padding:18px 14px 64px}}header.hero{{padding:24px 20px;border-radius:22px}}th,td{{padding:9px;font-size:.85rem}}}}
</style></head><body><main>
<header class="hero"><p class="eyebrow">CutMachine modern local editor · read-only review</p><h1>{esc(context.project["source"]["originalName"])}</h1><p class="muted">Deterministic plan, local media, explicit learning, one approval checkpoint.</p><p><span class="badge">{esc(status.upper())}</span> Recommended action: <strong>{esc(report["recommendation"])}</strong></p></header>
<nav aria-label="Editor sections"><a href="#draft">Preview</a><a href="#scenes">Timeline</a><a href="#transcript-warnings">Transcript</a><a href="#assets">Assets</a><a href="#color">Color</a><a href="#audio">Audio</a><a href="#learning">Learning</a><a href="#qc">QC</a><a href="#action">Action</a></nav>
...
<section id="learning"><h2>Local learning evidence</h2><div class="grid"><div class="card"><strong>Validated decisions</strong><br>{int(learning["events"])}</div><div class="card"><strong>Asset preferences</strong><br>{int(learning["assetPreferences"])}</div><div class="card"><strong>Caption corrections</strong><br>{int(learning["captionCorrections"])}</div><div class="card"><strong>Active style modes</strong><br>{esc(", ".join(cast(list[str], learning["activeStyleModes"])) or "defaults")}</div></div><p class="muted">{"Hash-bound local profiles are valid." if learning["valid"] else "No valid learning profile is active; deterministic defaults are in use."}</p></section>
