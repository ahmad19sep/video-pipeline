...
## 2026-07-16 - ADR-0055: The plan stage revalidates instead of regenerating

`generate_plan` rebuilt the baseline unconditionally, so the `plan_ready` rerun that every revision triggers silently discarded the change it was meant to apply: `request-revision` wrote the revised plan, invalidated `plan_ready`, and the next `resume` overwrote it with a fresh baseline and rendered an identical video while reporting success. Cowork imports and editor settings were lost the same way. Treat a persisted edit plan exactly like a resumed transcript under ADR-0013: revalidate it against project ownership, timeline version, duration, proxy identity, caption identity/timing, scene coverage, catalog membership, effect budgets, and source orientation, then keep it. Build the baseline only when no valid plan remains, so a fresh project still plans automatically and a stale or tampered plan still falls back deterministically. To force a new baseline, delete `planning/edit-plan.json` and rerun the stage.
